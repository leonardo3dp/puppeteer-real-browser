...
export async function connect({
  ...
  if (ignoreAllFlags === true) {
    ...
    flags[indexDisableFeatures] = `${flags[indexDisableFeatures]},AutomationControlled,GlobalMediaControls`;
    ...
    //extension
    const indexDisableExtension = flags.findIndex((flag) => flag.startsWith('--disable-extensions'));
    flags.splice(indexDisableExtension, 1);
	
    const indexDisableExtensionbg = flags.findIndex((flag) => flag.startsWith('--disable-component-extensions-with-background-pages'));
    flags.splice(indexDisableExtensionbg, 1);
